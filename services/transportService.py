...
class TransportService:
    def __init__(self, vehicle, travelTime, radius, rating_threshold, preferences, interests=[], current_timestamp='7:00'):
        ...
        self.visited_Transport = []
        # Convert environment variable to float (assuming it's in hours)
        self.default_Transport_wait_time = float(os.getenv("DEFAULT_Transport_WAITTIME", 1))  # Default to 1 hour if not set
    ...
    def find_best_Transport(self, Transports):
        # Convert all ratings to floats and distances to floats
        for Transport in Transports:
            Transport['rating'] = float(Transport['rating'])
            Transport['distance'] = float(Transport['distance'])
        
        # Sort Transports first by Rating (descending), then by Distance (ascending)
        best_Transport = sorted(Transports, key=lambda x: (-x['rating'], x['distance']))[0]
    ...
    def getCurrentTransportBasedOnCoordinates(self, latitude, longitude):
            ...
            # Combine the current interval and preferences for Transport search
            preference = [current_interval]
            for p in self.preferences:
                preference.append(p)
            #print(f"Preferences: {preference}")

            # Instantiate the TransportService to fetch the list of Transports
            resService = TransportService(
                lat=latitude,
                long=longitude, 
                radius=self.radius, 
                rating_threshold=self.rating_threshold, 
                preferences=preference, 
                visited_Transport=self.visited_Transport
            )
            
            # Get the best Transport from the list
            destination = self.find_best_Transport(resService.getTransports())
            #print(f"Best Transport Found: {destination}")

            # Add Transport wait time and travel time to the current state timestamp
            new_time = datetime.strptime(self.currentStateTimeStamp, "%H:%M") + \
                    timedelta(hours=self.default_Transport_wait_time) + \
            ...
            self.travelTime -= self.default_Transport_wait_time + self.calculateTimeToReachDestination(destination['distance'])
            #print(f"Remaining Travel Time: {self.travelTime} hours")

            # Update the current location after visiting the Transport
            self.current_location = destination.get('Transport').split('itp#')[1]
            #print(f"Updated Current Location: {self.current_location}")

            self.visited_Transport.append(self.current_location)
